...
public class AIMonster : AIBase
{
    ...
    protected float patrolRadius = 15f; //몬스터의 순찰 범위
    protected float WaitTime = 2f;  //대기 시간
    protected float MoveSpeed = 2f;  //몬스터 이동 속도
    protected float rotationSpeed = 3f; // 회전 속도
    ...
    //    Vector3 newPosition = Character.transform.position + direction * MoveSpeed * Time.deltaTime;
    //    newPosition.y = Character.transform.position.y; // 원래의 y 값을 유지

    //    Character.transform.position = newPosition; // 이동
